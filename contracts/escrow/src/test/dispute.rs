...
#[test]
fn assign_arbiter_allows_post_creation_dispute_path() {
    let (env, _contract_id, client) = setup_initialized();
    let client_addr = Address::generate(&env);
    let freelancer_addr = Address::generate(&env);
    let arbiter_addr = Address::generate(&env);
    ...
    assert!(client.assign_arbiter(&escrow_id, &client_addr, &arbiter_addr));

    let contract = client.get_contract(&escrow_id);
    assert_eq!(contract.arbiter, Some(arbiter_addr.clone()));

    assert!(client.raise_dispute(&escrow_id, &freelancer_addr));
    let disputed = client.get_contract(&escrow_id);
    assert_eq!(disputed.status, ContractStatus::Disputed);
}

#[test]
fn assign_arbiter_rejects_client_or_freelancer_as_arbiter() {
    let (env, _contract_id, client) = setup_initialized();
    let client_addr = Address::generate(&env);
    let freelancer_addr = Address::generate(&env);
    let escrow_id = client.create_contract(
    ...
    super::assert_contract_error(
        client.try_assign_arbiter(&escrow_id, &client_addr, &client_addr),
        EscrowError::InvalidArbiter,
    );
    super::assert_contract_error(
        client.try_assign_arbiter(&escrow_id, &freelancer_addr, &freelancer_addr),
        EscrowError::InvalidArbiter,
    );
}

#[test]
fn assign_arbiter_cannot_overwrite_existing_arbiter() {
    let (env, _contract_id, client) = setup_initialized();
    let client_addr = Address::generate(&env);
    let freelancer_addr = Address::generate(&env);
    let arbiter_addr = Address::generate(&env);
    let replacement_arbiter = Address::generate(&env);
    ...
    super::assert_contract_error(
        client.try_assign_arbiter(&escrow_id, &client_addr, &replacement_arbiter),
...
#[test]
fn assign_arbiter_requires_client_or_freelancer_caller() {
    let (env, _contract_id, client) = setup_initialized();
    let client_addr = Address::generate(&env);
    let freelancer_addr = Address::generate(&env);
    let arbiter_addr = Address::generate(&env);
    ...
    super::assert_contract_error(
        client.try_assign_arbiter(&escrow_id, &outsider, &arbiter_addr),
        EscrowError::UnauthorizedRole,
    );
}

#[test]
fn assign_arbiter_rejects_non_pre_dispute_status() {
    let (env, _contract_id, client) = setup_initialized();
    let client_addr = Address::generate(&env);
    let freelancer_addr = Address::generate(&env);
    let arbiter_addr = Address::generate(&env);
...
#[test]
fn assign_arbiter_respects_pause_and_finalization_gates() {
    let (env, _contract_id, client) = setup_initialized();
    let client_addr = Address::generate(&env);
    let freelancer_addr = Address::generate(&env);
    let arbiter_addr = Address::generate(&env);
    ...
    assert!(client.pause());
    super::assert_contract_error(
        client.try_assign_arbiter(&escrow_id, &client_addr, &arbiter_addr),
    ...
    assert!(client.deposit_funds(&escrow_id, &client_addr, &100_i128));
    assert!(client.raise_dispute(&escrow_id, &client_addr));
    assert!(client.finalize_contract(&escrow_id, &client_addr));
