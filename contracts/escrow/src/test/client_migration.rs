...
#[test]
fn cancel_pending_migration_success() {
    let env = Env::default();
    env.mock_all_auths();
    let client = register_client(&env);
    ...
    // event emitted
    assert!(has_event_with_topic(&env, &Symbol::new(&env, "client_migration_cancelled")));
}
...
fn cancel_without_pending_fails() {
    ...
    // attempt to cancel without a pending proposal
    assert_contract_error(
